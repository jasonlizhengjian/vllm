...
class MSACutlassDecodePlanCache:
    ...
    def _build_plan(
        ...
        *,
        ...
        from vllm.third_party.fmha_sm100 import fmha_sm100_plan
...
def _static_fallback_reason(
    ...
    *,
    ...
    if (
        query.shape != (total_q, _NUM_Q_HEADS, _HEAD_DIM)
        ...
        or topk.shape != (total_q, _NUM_KV_HEADS, _TOPK)
    ...
    if (
        ...
        or kv_cache.ndim != 4
        or kv_cache.shape[1:] != (_NUM_KV_HEADS, _PAGE_SIZE, 2 * _HEAD_DIM)
        or kv_cache.stride(-1) != 1
    ):
        return "KV cache is not interleaved HND FP8 E4M3"
    if any(
...
class _QueryBufferPool:
    ...
    def get(self, query: torch.Tensor) -> torch.Tensor:
        ...
        if self.current is None or tokens > self.current.capacity:
            ...
            self.current = _QueryBufferAllocation(
                ...
                query_fp8=torch.empty(
                    (capacity, _NUM_Q_HEADS, _HEAD_DIM),
...
class MSACutlassSparseDecodeRunner:
    """Layer-owned FP8 query buffers for the published MiniMax kernel."""
    ...
    def try_decode(
        ...
        *,
        ...
        if reason is not None:
            if requested() and reason != "MSA decode metadata is unavailable":
                logger.warning_once(
        ...
        from vllm.third_party.fmha_sm100 import fmha_sm100
        ...
        logger.info_once(
            "MiniMax M3 sparse decode dispatched the published CUTLASS "
            "fmha_sm100 per-token sparse kernel"
        )
        return True
